...
func (_q *UserSubscriptionQuery) QuerySubscriptionRecords() *SubscriptionRecordQuery {
...
func (_q *UserSubscriptionQuery) WithSubscriptionRecords(opts ...func(*SubscriptionRecordQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withSubscriptionRecords; query != nil {
		if err := _q.loadSubscriptionRecords(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.SubscriptionRecords = []*SubscriptionRecord{} },
			func(n *UserSubscription, e *SubscriptionRecord) {
...
func (_q *UserSubscriptionQuery) loadSubscriptionRecords(ctx context.Context, query *SubscriptionRecordQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionRecord)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionrecord.FieldSubscriptionID)
	}
	query.Where(predicate.SubscriptionRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usersubscription.SubscriptionRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
