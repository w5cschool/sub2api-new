...
func (_q *GroupQuery) QuerySubscriptionRecords() *SubscriptionRecordQuery {
...
func (_q *GroupQuery) WithSubscriptionRecords(opts ...func(*SubscriptionRecordQuery)) *GroupQuery {
	query := (&SubscriptionRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSubscriptionRecords; query != nil {
		if err := _q.loadSubscriptionRecords(ctx, query, nodes,
			func(n *Group) { n.Edges.SubscriptionRecords = []*SubscriptionRecord{} },
			func(n *Group, e *SubscriptionRecord) {
...
func (_q *GroupQuery) loadSubscriptionRecords(ctx context.Context, query *SubscriptionRecordQuery, nodes []*Group, init func(*Group), assign func(*Group, *SubscriptionRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionrecord.FieldGroupID)
	}
	query.Where(predicate.SubscriptionRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.SubscriptionRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
