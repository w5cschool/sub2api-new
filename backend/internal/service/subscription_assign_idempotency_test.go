...
func (s *subscriptionUserSubRepoStub) ResetUsageWindows(ctx context.Context, id int64, resetDaily, resetWeekly, resetMonthly bool, windowStart time.Time) error {
	if resetDaily {
		if err := s.ResetDailyUsage(ctx, id, nil, windowStart); err != nil {
			return err
		}
	}
	if resetWeekly {
		if err := s.ResetWeeklyUsage(ctx, id, nil, windowStart); err != nil {
			return err
		}
	}
	if resetMonthly {
		if err := s.ResetMonthlyUsage(ctx, id, nil, windowStart); err != nil {
			return err
		}
	}
	return nil
