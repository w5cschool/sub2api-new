...
func (s *subscriptionUserSubRepoStub) ResetUsageWindows(ctx context.Context, id int64, resetDaily, resetWeekly, resetMonthly bool, dailyStart, periodicStart time.Time) error {
	if resetDaily {
		if err := s.ResetDailyUsage(ctx, id, nil, dailyStart); err != nil {
			return err
		}
	}
	if resetWeekly {
		if err := s.ResetWeeklyUsage(ctx, id, nil, periodicStart); err != nil {
			return err
		}
	}
	if resetMonthly {
		if err := s.ResetMonthlyUsage(ctx, id, nil, periodicStart); err != nil {
			return err
		}
	}
	return nil
